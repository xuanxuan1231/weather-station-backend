...
def add_temp():
    ...
    if not temp:
        return flask.jsonify({"status": 1, "error": "temp is required"}), 400
    data.add_temp(temp, time)
    return flask.jsonify({"status": 0, "temp": temp, "time": time}), 200
...
def get_temp():
    ...
    if "error" in temp_data:
        return flask.jsonify({"status": 1, "error": temp_data["error"]}), 500
...
def add_humidity():
    ...
    time = datetime.datetime.now().isoformat()
    if not humidity:
        return flask.jsonify({"status": 1, "error": "humidity is required"}), 400
    data.add_humidity(humidity, time)
    return flask.jsonify({"status": 0, "humidity": humidity, "time": time}), 200
...
def get_humidity():
    ...
    if "error" in humidity_data:
        return flask.jsonify({"status": 1, "error": humidity_data["error"]}), 500
    return flask.jsonify(
        {
            "status": 0,
...
def add_pressure():
    pressure = flask.request.json["pressure"]
    time = datetime.datetime.now().isoformat()
    if not pressure:
        return flask.jsonify({"status": 1, "error": "pressure is required"}), 400
    data.add_pressure(pressure, time)
    return flask.jsonify({"status": 0, "pressure": pressure, "time": time}), 200
...
def get_pressure():
    ...
    if "error" in pressure_data:
        return flask.jsonify({"status": 1, "error": pressure_data["error"]}), 500
    return flask.jsonify(
        {
            "status": 0,
...
def add_pm2_5():
    pm2_5 = flask.request.json["pm2_5"]
    time = datetime.datetime.now().isoformat()
    if not pm2_5:
        return flask.jsonify({"status": 1, "error": "pm2_5 is required"}), 400
    data.add_pm2_5(pm2_5, time)
    return flask.jsonify({"status": 0, "pm2_5": pm2_5, "time": time}), 200
...
def get_pm2_5():
    ...
    if "error" in pm2_5_data:
        return flask.jsonify({"status": 1, "error": pm2_5_data["error"]}), 500
    return flask.jsonify(
